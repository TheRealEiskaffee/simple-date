    ...
    public format(format ?: string) {
        ...
        if(format) {
            ...
            if(new RegExp(/\bYYYY\b/).test(response)) {
                response = response.replace(/\bYYYY\b/g, date.toISOString().substring(0, 4)); //YYYY (2023)
            }
            
            if(new RegExp(/\bMM\b/).test(response)) {
                response = response.replace(/\bMM\b/g, date.toISOString().substring(5, 7)); //MM (05)
            }

            if(new RegExp(/\bDD\b/).test(response)) {
                response = response.replace(/\bDD\b/g, date.toISOString().substring(8, 10)); //DD (15)
            }
            
            if(new RegExp(/\bHH\b/).test(response)) {
                response = response.replace(/\bHH\b/g, date.toISOString().substring(11, 13)); //HH (23)
            }

            if(new RegExp(/\bmm\b/).test(response)) {
                response = response.replace(/\bmm\b/g, date.toISOString().substring(14, 16)); //mm (14)
            }

            if(new RegExp(/\bss\b/).test(response)) {
                response = response.replace(/\bss\b/g, date.toISOString().substring(17, 19)); //ss (42)
            }

            if(new RegExp(/\bdddd\b/).test(response)) {
                response = response.replace(/\bdddd\b/g, date.toLocaleString(this.settings.locale, { weekday: 'long', timeZone: this.settings.timeZone })); //dddd (Saturday)
            }

            if(new RegExp(/\bdd\b/).test(response)) {
                response = response.replace(/\bdd\b/g, date.toLocaleString(this.settings.locale, { weekday: 'short', timeZone: this.settings.timeZone })); //dd (Sa)
            }

            if(new RegExp(/\bMMMM\b/).test(response)) {
                response = response.replace(/\bMMMM\b/g, date.toLocaleString(this.settings.locale, { month: 'long', timeZone: this.settings.timeZone })); //MMMM (January)
            }

            if(new RegExp(/\bMMM\b/).test(response)) {
                response = response.replace(/\bMMM\b/g, date.toLocaleString(this.settings.locale, { month: 'short', timeZone: this.settings.timeZone })); //MMM (Jan)
            }
        } else {
