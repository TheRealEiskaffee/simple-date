...
type setOptions = { day ?: number, month ?: number, year ?: number, hours ?: number, minutes ?: number, seconds ?: number, milliseconds ?: number };
...
class SimpleDate {
    ...
    /**
     * Sets date parts via partial object (month 1-based) or single unit/value pair; clamps time inputs to valid ranges (hours 0-23, minutes/seconds 0-59).
     * @param partOrOptions Partial object `{ day, month, year, hours, minutes, seconds, milliseconds }` or a unit string.
     * @param value Value applied when the first argument is a unit string.
     */
    public set(unit : adoptUnit, value : number) : SimpleDate;
    public set(options : setOptions) : SimpleDate;
    public set(unitsOrOptions : adoptUnit | setOptions, value ?: number) : SimpleDate {
        const newDate = this.date ? new Date(this.date) : new Date();
        const clamp = (val : number, max : number) => val < 0 ? 0 : val > max ? max : val;
        const apply = (options : setOptions) => {
