...
class SimpleDate {
    ...
    endOf(unitOf) {
        ...
        switch (unitOf) {
            ...
            case 'week':
                //TODO ist noch buggy
                let startDate = new Date(newDate.setDate(newDate.getDate() - newDate.getDay() === 0 ? 0 : newDate.getDay() - 1)), endDate = new Date(startDate);
     ...
     */
    format(format) {
        let response = this.date instanceof Date && !isNaN(this.date.getTime()) ? this.date.toISOString().substring(0, 10) : undefined, date = new Date(this.date);
        if (format) {
